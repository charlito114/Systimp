...
--
-- Host: 127.0.0.1
-- Generation Time: Nov 17, 2019 at 10:06 AM
...
(0001, 0001, 10000000000, 'pvc blue', 'moldex', 'pipes 3 meters', '1/2', 50, 50, 100);
...
IF((new.QuantityIssued > new.Quantity)  OR (prodavail < new.QuantityIssued)) THEN 
...
(0001, '2019-11-17', 'aly', 'laguna', 7000, 'Pending');
...
(10000000000, 'pvc blue', 'moldex', 'pipes 3 meters', '1/2', 50, 10, 50, 100, 'Available'),
(10000000001, 'pvc blue', 'moldex', 'male adaptor', '1/2', 45, 10, 35, 50, 'Available'),
(10000000002, 'pvc blue', 'moldex', 'reducer', '2 X 1 ', -5, 10, 195, 45, 'Available'),
(10000000003, 'pvc blue', 'tian', 'cap', '2', 30, 10, 0, 150, 'Available'),
(10000000004, 'pvc blue', 'tian', 'plug', '3', 0, 10, 20, 20, 'Available'),
(10000000005, 'pvc blue', 'tian', 'threaded tee ', '4', 40, 10, 0, 90, 'Available'),
(10000000006, 'pvc faucet', 'n/a', 'plain bibb', '1/2', 50, 10, 0, 80, 'Available'),
(10000000007, 'pvc valves', 'n/a', 'ball valve', '1/2', 9, 10, 50, 70, 'Available'),
(10000000008, 'pvc valves', 'n/a', 'check valve', '1', -5, 10, 55, 60, 'Available'),
(10000000009, 'ppr white', 'era', 'gate valve', '3', -9, 10, 20, 35, 'Available'),
(10000000010, 'ppr white', 'era', 'ball valve', '2', 0, 10, 10, 25, 'Available'),
(10000000011, 'ppr white', 'vesbo', 'cap', '1', 10, 10, 10, 150, 'Available'),
(10000000012, 'ppr white', 'vesbo', 'pipes 4 meters', '1/2', 25, 10, 20, 120, 'Available'),
(10000000013, 'sanitary orange', 'emerald', 'tee reducer', '4 X 2', 55, 10, 0, 130, 'Available'),
(10000000014, 'sanitary orange', 'neltex', 'wye', '4', 10, 10, 0, 125, 'Available'),
(10000000015, 'sanitary orange', 'era', 'Ball Valve', '1/2', -10, 10, 30, 90, 'Discontinued');
...
(0001, '2019-11-17', '', '', 5000, 5600, 0001, 'Ongoing');
...
(0001, 10000000000, 'pvc blue', 'moldex', 'pipes 3 meters', '1/2', 50, 50, 50, 100, 5000, 'Fully Issued'),
(0001, 10000000001, 'pvc blue', 'moldex', 'male adaptor', '1/2', 40, 45, 0, 50, 2000, 'Processing');
...
(0001, 0001, 10000000000, 'pvc blue', 'moldex', 'pipes 3 meters', '1/2', 100, 50, 0, 50, 100, 5000);
...
--
-- Dumping data for table `temporarypurchasing`
--

INSERT INTO `temporarypurchasing` (`PONum`, `ProductCode`, `Category`, `Brand`, `ProductDesc`, `Size`, `SuggestedQuantity`, `QuantitytobeOrdered`) VALUES
(0001, 10000000004, 'pvc blue', 'tian', 'plug', '3', 10, 10);
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `pendingpo`  AS  select `p`.`PONum` AS `PONum`,`p`.`Date` AS `Date`,`p`.`SupplierName` AS `SupplierName`,`p`.`Status` AS `Status`,count(`pd`.`PONum`) AS `totalItems`,sum(`pd`.`status` = 'Processing') AS `pendingItems`,round(count(`pd`.`PONum`) - sum(`pd`.`status` = 'Processing') / count(`pd`.`PONum`),2) * 100 AS `completion` from (`p_purchasingmanagement` `p` join `p_podetails` `pd` on(`p`.`PONum` = `pd`.`PONum`)) where `p`.`Status` = 'Pending' group by `pd`.`PONum` ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `pendingso`  AS  select `o`.`Date` AS `Date`,`o`.`SONum` AS `SONum`,`o`.`CustomerName` AS `CustomerName`,count(`od`.`SONum`) AS `totalItems`,sum(`od`.`Status` = 'Processing') AS `pendingItems`,(count(`od`.`SONum`) - sum(`od`.`Status` = 'Processing') / count(`od`.`SONum`)) * 100 AS `completion` from (`ordermanagement` `o` join `salesorderdetails` `od` on(`o`.`SONum` = `od`.`SONum`)) where `o`.`Status` = 'Pending' ;
...
--
ALTER TABLE `ordermanagement`
  MODIFY `SONum` int(4) UNSIGNED ZEROFILL NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=2;
...
--
ALTER TABLE `salesmanagement`
  MODIFY `invoiceNum` int(4) UNSIGNED ZEROFILL NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=2;
