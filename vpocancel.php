...
if(isset($_POST['cancel']))
{
    ...
    while($row = $result2->fetch_assoc()) {
        ...
        echo "\t<tr><td >" . $row['ProductCode'] . "</td><td>" . $row['Category'] . "</td><td>"  .  $row['Brand'] . "</td><td>" .
         $row['ProductDesc'] . "</td><td>" . $row['Size'] . "</td><td>" . $row['Quantity'] . "</td><td>" . $row['ToReceive'] . "</td><td>" . $row['status'] . 
          "</td><td><select name = 'cancelreason'> 
          <option value='Timeframe'>Timeframe</option>
          <option value='Stock'>Insufficient Stock</option></td> <td><button type = 'submit' name = 'submit' formaction = 'processcancel.php' value = '" . $row['ProductCode'] . "' > Submit </button></td><td><button type = 'submit' name = 'back' formaction = 'view_purchase_order.php'>Back</td></tr>\n";
        }
        else{
